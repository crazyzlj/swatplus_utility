...
DAG_FILE_TO_SUBMIT="ctrl_calibration.dag"
...
if grep -q "continue" "$SIGNAL_FILE"; then
    ...
    # Read iterative index from "iteration.state"
    iter_index=$(cat $STATE_FILE)
    echo "Read current iteration number from $STATE_FILE: iter_index"
    nextiter_index=$(($iter_index+1))

    echo "$(date): Executing: condor_submit_dag -update_submit -insert_env "iter_index=$iter_index" -insert_env "nextiter_index=$nextiter_index" $DAG_LOG_DIR $DAG_FILE_TO_SUBMIT" >> "$LOG_FILE"
    ...
    condor_submit_dag -update_submit -insert_env "iter_index=$iter_index" -insert_env "nextiter_index=$nextiter_index" "$DAG_FILE_TO_SUBMIT" >> "$LOG_FILE" 2>&1
    SUBMIT_EXIT_CODE=$?
