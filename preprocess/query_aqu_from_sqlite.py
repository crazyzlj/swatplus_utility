# 用Python实现如下功能：
# 1. 从QSWAT+构建的sqlite数据库中，按要求查询符合条件的aqu，并输出aqu的编号
# 2. 用户指定一个或多个子流域编号，如[30, 31, 32]
# 3. 在sqlite数据库的gis_aquifers表中，根据subbasin字段（即子流域编号）查询对应的aqu编号（即id）
# 5. 将符合条件的aqu编号排序,输出个数和编号
import sqlite3
import os
import json
from typing import List, Optional, Tuple, Dict, Any

def query_aqu_by_subbasins(db_path: str,
                           subbasins: List[int]) -> Tuple[int, List[int]]:
    """
    从 QSWAT+ SQLite 数据库中，根据子流域编号
    查询对应的 Channel 编号。
    """
    ...
    try:
        ...
        aqu_ids = [row[0] for row in results]

        return len(aqu_ids), aqu_ids

    except sqlite3.Error as e:
        if "no such column" in str(e):
            print(f"数据库查询错误: {e}")
            print("错误提示: 请检查 'gis_aquifers' 表中是否"
                  "存在名为 'subbasin' 的字段。")
        else:
...
def run_and_save_aquifers_combinations(db_path: str,
                                       sub_dict: Dict[str, Optional[List[int]]],
                                       output_file_path: str):
    """
    遍历所有子流域组合，运行查询，并保存为 JSON。
    ...
    for sub_key, sub_values in processed_sub_dict.items():
            # 2. 生成组合名称
            combination_name = f"{sub_key}"

            print(f"正在处理: {combination_name}...")

            # 3. 执行查询
            count, channel_list = query_aqu_by_subbasins(
                    db_path,
                    sub_values
            )
            if count == 0:
            ...
            print(f" -> 完成: {combination_name} | 找到 {count} 个 Aquifers")
...
if __name__ == "__main__":
    ...
    OUTPUT_JSON_FILE = r"D:\data_m\manitowoc_test30m\manitowoc_test30mv4\subbasin_updown_relationships\aqu_combinations.json"
